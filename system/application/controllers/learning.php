...
class Learning extends Controller {
	...
	function mahasiswa()
	{
		...
		$tot_hal = $this->db->select("*")->join("tblmahasiswa","tblmahasiswa.id_mahasiswa=tbllogin.username","left")->get_where("tbllogin",array("status"=>"Mahasiswa"));
		$query = $this->db->select("*")->join("tblmahasiswa","tblmahasiswa.id_mahasiswa=tbllogin.username","left")->get_where("tbllogin",array("status"=>"Mahasiswa"),$limit_dosen,$offset_dosen);

      	 	$config['base_url'] = base_url() . '/index.php/learning/mahasiswa/';
        	$config['total_rows'] = $tot_hal->num_rows();
        	$config['per_page'] = $limit_dosen;
			$config['uri_segment'] = 3;
	    	$config['first_link'] = 'Awal';
		...
		$this->load->view('e-learning/bg_menu',$data);
		$this->load->view('e-learning/bg_kiri',$data);
		$this->load->view('e-learning/mahasiswa',$data2);
		$this->load->view('e-learning/bg_kanan',$data);
		$this->load->view('e-learning/bg_footer');
	}
	function detail_dosen($param)
	{
		$this->load->model('Learning_model');
		$this->load->library('Pagination');
		$page=$this->uri->segment(3);
		...
		$det_dosen=$this->db->get_where("tbldosen",array("id_dosen"=>$param))->row();

		$data2['dosen'] = $det_dosen->dosen;
		$data2['alamat'] = $det_dosen->alamat;
		$data2['tempat_lahir'] = $det_dosen->tempat_lahir;
		$data2['tgl_lahir'] = $det_dosen->tgl_lahir;
		$data2['pend_terakhir'] = $det_dosen->pend_terakhir;
		...
		$this->load->view('e-learning/bg_menu',$data);
		$this->load->view('e-learning/bg_kiri',$data);
		$this->load->view('e-learning/detail_dosen',$data2);
		$this->load->view('e-learning/bg_kanan',$data);
		$this->load->view('e-learning/bg_footer');
	}
	function detail_mahasiswa($param)
	{
		$this->load->model('Learning_model');
		$this->load->library('Pagination');
		$page=$this->uri->segment(3);
		...
		$det_dosen=$this->db->get_where("tblmahasiswa",array("id_mahasiswa"=>$param))->row();

		$data2['nama'] = $det_dosen->nama;
		$data2['alamat'] = $det_dosen->alamat;
		$data2['angkatan'] = $det_dosen->angkatan;
		$data2['tempat_lahir'] = $det_dosen->tempat_lahir;
		$data2['tgl_lahir'] = $det_dosen->tgl_lahir;
		$data2['jurusan'] = $det_dosen->jurusan;
		...
		$this->load->view('e-learning/bg_menu',$data);
		$this->load->view('e-learning/bg_kiri',$data);
		$this->load->view('e-learning/detail_mahasiswa',$data2);
		$this->load->view('e-learning/bg_kanan',$data);
		$this->load->view('e-learning/bg_footer');
	}
	function mi()
...
//==========================================Memakai Highslide Javascript===========================================
	...
	function uploadfoto()
	{
		...
		$this->load->view('e-learning/upload_foto',$data);
	}
